<!--
	This load function is not actually used, but something has to reference the
	/sitemap.xml endpoint so that it is generated, i.e. this is a HACK!
-->
<script context="module" lang="ts">
	export async function load({ fetch }) {
		const res = await fetch('/sitemap.xml');
		return {};
	}
</script>

<script lang="ts">
	import { seo } from '$lib/seo/store';

	seo.reset();
	$seo.title = 'Page not found';
	$seo.description = "We are sorry that you have landed here, and hope you'll keep browsing.";
</script>

<section>
	<h1>
		<span class="text-v-lilac">New website.</span>
		<span class="text-v-pink">New routes.</span>
	</h1>
	<h3>We are sorry that you have landed here. Check the header for links to common pages.</h3>
</section>
